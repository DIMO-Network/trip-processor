...
install: $(APPS)
	@mkdir -p bin
	@cp $(PATHINSTBIN)/trip-processor ./bin/
...
docker: deps
	@docker build -f ./resources/docker/Dockerfile . -t dimozone/trip-processor:$(VER_CUT)
	@docker tag dimozone/trip-processor:$(VER_CUT) dimozone/trip-processor:latest

docker-cgo: deps
	@docker build -f ./resources/docker/Dockerfile.cgo . -t dimozone/trip-processor:$(VER_CUT)-cgo
	@docker tag dimozone/trip-processor:$(VER_CUT)-cgo dimozone/trip-processor:latest-cgo
...
test: $(APPS)
	...
	@$(PATHINSTBIN)/trip-processor test ./config/test/...
